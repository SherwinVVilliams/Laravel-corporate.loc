	 ...
	 <div class="sticky hentry hentry-post blog-big group">
		...
		<!-- post title -->
			...
			<div class="image-wrap">
				<img src="/{{ env('THEME') }}/images/articles/{{ $article->img->max }}" alt="001" title="001" />        
			</div>
			<p class="date">
				<span class="month"> <!--$article->created_at->format('M') ? $article->created_at->format('M') : 'Sep' --><p>Sep</p></span>
				<span class="day"><!-- $article->created_at->format('d') ? $article->created_at->format('d') : '24'  --> <p>24</p></span>
			</p>
		</div>
		<!-- post meta -->
