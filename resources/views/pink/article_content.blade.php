 <div id="content-single" class="content group">
	<div class="hentry hentry-post blog-big group ">
		...
		<h1 class="post-title"><a href="#">{{ $article->title }}</a></h1>
			...
			<p class="date">
			<span class="month">{{ ($article->created_at != null ) ? $article->created_at->format('M') : 'Sep' }}</span>
			<span class="day">{{ ($article->created_at != null ) ? $article->created_at->format('d') : '25' }}</span>
			</p>
				                   
		</div>
				                <!-- post meta -->
		<div class="meta group">
			<p class="author"><span>by <a href="#" title=" Post by {{ $article->user->name }}" rel="author">{{ $article->user->name }}</a></span></p>
